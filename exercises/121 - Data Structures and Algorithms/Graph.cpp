...
private:
    ...
    std::vector<bool> visitedNodes;         // marks if node is already visited
...
std::pair<std::vector<pint>, uint> Graph::dijkistra(int srcNode, int desNode, bool isOut, bool isFirst) {
    ...
    int outdist = this->distArray[desNode];
    if (noPath) {
        outdist = -1;
    ...
    if (isOut) {
        output(std::make_pair(shortest_nodePath, outdist));
        return out;
    }
    ...
    if (isFirst) {
        ...
        std::cout << "\nIt has length " << outdist << '\n';
    }
    out = std::make_pair(shortest_edgePath, outdist);
    return out;
}
...
void Graph::yen(int srcNode, int desNode) {
    ...
    while (firstShortest.second == shortestDistance) {
        ...
        for (uint i = 0; i < allShortest[numShortest-1].size(); ++i) {
            ...
            // In case the shortest path appended is noPath
            if (spurNode == -1) {
                break;
            }
            ...
            // Append combined paths to potentialShortest list if it's a noPath
            if (noPath == -1) {
                continue;
            }
    ...
    // Remove all edges in allShortest from the graph
    // Find shortest path with the modified graph
