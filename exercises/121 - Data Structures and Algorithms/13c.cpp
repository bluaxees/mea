...
void Graph::toposort_recur(int current, std::queue<int> &q) { 
    this->visitedNodes[current] = true; 
...
void Graph::toposort() { 
    ...
    while (!q.empty()) { 
        std::cout << this->nodeLabels[q.front()] << " "; 
        q.pop();
    }
}
...
void Graph::init(std::string inputFile) {
    ...
    for(uint i = 0; i < this->adjList.size(); ++i) {
        std::string label_a  = "";
        std::string label_b  = "";
        if (inputFile == "exercise13.in") {
            fh >> label_a >> label_b;
            this->nodeLabels[i] = label_a + " " + label_b;
        }
        else {
            fh >> label_a;
            this->nodeLabels[i] = label_a;
        }
    }
    int endOne = 0;
    int endTwo = 0;
